...
var tpl = fs.readFileSync(path + '/template.html', 'utf8');

files.forEach(function(v, i){
  if(v.endsWith('.md')) {
    //links += util.format(html_a, v, v);
    var title = '頁面：<a href="index.html?page=' + v + '">' + v + '</a>';
    body += util.format(html_body, v, title, mkup(fs.readFileSync(path + '/' + v, 'utf8')));
  }
});

var out = util.format(tpl, links + '<hr/>' + body);
fs.writeFile(path + '/onepage.html', out, 'utf8', function(err){
